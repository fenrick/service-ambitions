...
class CountingSession:
    def __init__(self, payload: PlateauFeaturesResponse | None = None, *, raise_invalid: bool = False) -> None:
        self.calls = 0
...
def test_no_parallel_model_calls(monkeypatch) -> None:
    ...
    monkeypatch.setattr("asyncio.gather", lambda *a, **k: (_ for _ in ()).throw(AssertionError("gather not allowed")))
    monkeypatch.setattr("asyncio.create_task", lambda *a, **k: (_ for _ in ()).throw(AssertionError("create_task not allowed")))
    ...
    class DummySession:
        ...
        def add_parent_materials(self, service_input: ServiceInput) -> None:  # noqa: D401
            return None
    ...
    # Features payload (per plateau) will be provided by PlateauRuntime; we bypass it by stubbing
    # generate_plateau_async to avoid actual model calls and keep focus on scheduling logic.
    session = DummySession([desc_payload])
    gen = PlateauGenerator(cast(ConversationSession, session), use_local_cache=False, cache_mode="off")
    service = ServiceInput(service_id="svc", name="Service", customer_type="retail", description="d", jobs_to_be_done=[{"name": "job"}])

    async def fake_generate_plateau_async(self, runtime, *, session=None):  # noqa: ANN001
        runtime.set_results(features=[], mappings={})
        return runtime

    monkeypatch.setattr(PlateauGenerator, "generate_plateau_async", fake_generate_plateau_async)
